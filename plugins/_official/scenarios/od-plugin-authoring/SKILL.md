...
- `readable-studio.json` with valid plugin metadata, `od.kind`, mode, task kind, capabilities, and inputs when needed.
...
- Treat `SKILL.md` as the canonical behavior description. `readable-studio.json` should describe how Open Design installs, applies, and presents that behavior.
