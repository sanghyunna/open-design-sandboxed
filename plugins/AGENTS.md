...
- Keep runnable plugin examples portable: every example should have a `SKILL.md`; add `readable-studio.json` only as the OD sidecar.
- Keep `SKILL.md` bodies free of OD-only marketplace metadata. Put OD display, inputs, preview, pipeline, capabilities, and source information in `readable-studio.json`.
