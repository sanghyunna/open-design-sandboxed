...
`--headless` makes `install`, `start`, `stop`, `uninstall`, and `cleanup` operate on the headless entry (`@readable-studio/packaged/dist/headless.mjs`) instead of the AppImage. Headless mode runs daemon + web without Electron.
