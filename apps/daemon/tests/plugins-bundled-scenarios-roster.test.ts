...
// using the `od-<taskKind>` id rule, so documented sibling scenarios can
...
describe('plugins/_official/scenarios roster', () => {
  ...
  it('od-default is hidden and asks for task type through a GenUI surface', async () => {
    const manifestPath = path.join(scenariosRoot, 'od-default', 'readable-studio.json');
    const manifest = JSON.parse(await readFile(manifestPath, 'utf8'));
  ...
  it('od-new-generation declares the default craft rails for anti-slop HTML output', async () => {
    const manifestPath = path.join(scenariosRoot, 'od-new-generation', 'readable-studio.json');
    const manifest = JSON.parse(await readFile(manifestPath, 'utf8'));
    expect(manifest.readable.context?.craft).toEqual(
      expect.arrayContaining(['typography', 'color', 'anti-ai-slop']),
    );
