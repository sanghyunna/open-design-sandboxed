...
export function readPosthogConfig(
  ...
  // unconditionally. We intentionally ignore POSTHOG_KEY / POSTHOG_HOST so no
  // daemon-emitted analytics ever leave the machine. The env plumbing above
  // (and readTelemetryEnvironment) is retained so /api/analytics/config still
  // produces a valid `{enabled:false,...}` response shape; the cascade
  // (createAnalyticsService -> NOOP_SERVICE) follows automatically.
  return null;
}
