...
import { buildDocumentPreview, PreviewHttpError } from './document-preview.js';
...
export async function startHostedServer(
  ...
  retireGenerationResources = ({ generation, userKey }) => {
    ...
    previewScopes.revokeGeneration({ generation, userKey });
  ...
  const parseHostedUpload: RequestHandler = async (request, response, next) => {
    const declaredBytes = request.headers['content-length'];
    if (declaredBytes == null || !/^(?:0|[1-9]\d*)$/u.test(declaredBytes)) {
      next(new HostedHttpError('BAD_REQUEST', 'hosted content length is required', 400));
      return;
    }
    ...
    let reservation: ReturnType<typeof bodyCapacity.reserve> | null = null;
    ...
    const timer = setTimeout(() => {
      ...
      void intake?.cleanup();
      if (!response.headersSent) {
        apiFailure(response, 408, 'BAD_REQUEST', 'hosted upload timed out');
        response.once('finish', () => request.destroy());
      } else {
        request.destroy();
      }
  ...
  app.get(
    ...
    async (request, response) => {
      const result = await dispatchContent(requestState(response), {
        ...
        readonly file: { readonly mime: string };
      };
      response
  ...
  app.get(
    ...
    async (request, response) => {
      const projectId = routeParam(request, 'id');
      const binding = previewScopes.resolve(routeParam(request, 'scope'), { projectId });
      if (binding == null) {
...
function uploadState(response: Response): HostedUploadRequestState {
  ...
  if (state == null) {
    throw new HostedHttpError('BAD_REQUEST', 'hosted upload intake is unavailable', 400);
  }
  return state;
}
...
function wildcardParam(request: Request, name: string): string {
  ...
  throw new HostedHttpError('BAD_REQUEST', 'hosted wildcard path is invalid', 400);
}
...
function requiredQueryString(request: Request, key: string): string {
  ...
  if (typeof value !== 'string') {
    throw new HostedHttpError('BAD_REQUEST', 'hosted query is invalid', 400);
  }
  return value;
}
...
function optionalQueryInteger(
  ...
  if (!/^(?:0|[1-9]\d*)$/u.test(value)) {
    throw new HostedHttpError('BAD_REQUEST', 'hosted query integer is invalid', 400);
  }
