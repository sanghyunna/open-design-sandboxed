...
export const ko: Dict = {
  ...
  'settings.onboardingCreateBody': '원하는 사이트, 앱, 덱, 이미지 또는 동영상을 설명하세요. Readable Studio이 프로젝트를 생성하고 작업물을 편집 가능한 상태로 유지합니다.',
  ...
  'settings.agentInstall.stepAuth': 'Readable Studio으로 돌아오기 전에 공급자 CLI에서 인증(로그인 또는 API 자격 증명 추가)을 완료하세요.',
  ...
  'settings.mcpServerHint': 'Readable Studio을 코딩 에이전트용 MCP 서버로 노출합니다.',
  ...
  'settings.privacyConsentLead': 'Readable Studio은 제품 개선을 위해 사용 데이터를 팀과 공유할 수 있습니다. 여기에는 다음이 포함됩니다:',
  ...
  'settings.mcpDaemonError': '설치 경로를 확인하기 위해 로컬 데몬에 연결하지 못했습니다 ({error}). Readable Studio이 실행 중인지 확인한 후 이 패널을 다시 열어 주세요.',
  ...
  'settings.mcpRunningNote': 'MCP 도구 호출이 성공하려면 Readable Studio이 실행 중이어야 합니다. Readable Studio을 열기 전에 코딩 Agent를 시작했다면, 실행 중인 데몬에 연결할 수 있도록 Agent를 다시 시작하세요.',
  ...
  'integrations.lede': '외부 시스템을 연결하고, MCP 도구를 Agent 루프에 가져오고, 다른 IDE, 스크립트, 자동화에서 Readable Studio을 사용하세요.',
  ...
  'mcpClient.daemonError': '로컬 데몬에 연결할 수 없습니다. Readable Studio이 실행 중인지 확인한 후 이 패널을 다시 여세요.',
  ...
  'useEverywhere.modalSubtitle': 'Readable Studio을 어떤 IDE, Agent, 스크립트에든 적용하세요 — CLI, HTTP, MCP, Skills. “Agent용 가이드 복사”를 사용해 Claude Code, Codex, Cursor, openclaw, hermes에 붙여 넣어 모든 것을 설정하세요.',
  ...
  'useEverywhere.section.overview.intro': 'Readable Studio은 단순한 창 이상입니다 — 로컬 권한 데몬(`od`)에 Skills + Design-Systems + Atoms 레지스트리를 더한 것입니다. 머신에서 실행되면 어떤 코드 Agent(Claude Code, Codex, Cursor, OpenCode/openclaw, Hermes, 직접 만든 스크립트)든 네 가지 호환 가능한 인터페이스를 통해 생성을 구동하고, 프로젝트를 검사하고, 디자인 아티팩트를 만들 수 있습니다.',
  'useEverywhere.section.overview.bullet1': 'CLI — 헤드리스 스크립트, CI, 셸 자동화를 위한 `od <command>`.',
  'useEverywhere.section.overview.bullet2': 'MCP 서버 — Readable Studio을 Model Context Protocol 서버로 연결해 MCP 지원 Agent가 skills를 나열하고, 시나리오를 실행하고, 아티팩트를 읽을 수 있게 합니다.',
  ...
  'useEverywhere.section.cli.bullet2': '`od project create` + `od run start` - create a project, send a message, and stream the run.',
  ...
  'useEverywhere.section.mcp.intro': 'Readable Studio에는 Model Context Protocol 서버(`od mcp`)가 포함되어 있어, MCP 지원 클라이언트 — Cursor, Claude Code, Antigravity, VS Code Copilot Chat, openclaw, hermes — 가 수동으로 셸을 거치지 않고도 Readable Studio 도구(skills 나열, 미리 보기 렌더링, 미디어 생성, 플러그인 실행)를 발견할 수 있습니다. 데몬은 각 주요 클라이언트에 대해 바로 붙여 넣을 수 있는 설치 스니펫을 `GET /api/mcp/install-info`를 통해 게시합니다.',
  ...
  'useEverywhere.section.skills.heading': '어떤 Agent를 위한 드롭인 Skills — Readable Studio이 실행되고 있지 않아도',
  'useEverywhere.section.skills.intro': 'Skill은 Claude 호환 `SKILL.md`(YAML 프런트 매터 + 본문)를 포함하는 디렉터리입니다. Readable Studio은 `readable:` 네임스페이스(`mode`, `preview`, `design_system`, `inputs`, …)로 형식을 확장하므로 동일한 아티팩트를 Readable Studio 내부와 Claude Code, Codex, openclaw, hermes 같은 일반 Agent 양쪽에서 사용할 수 있습니다. 발견은 우선순위 체인을 따르므로 프로젝트가 자체 skills를 재정의할 수 있습니다.',
  ...
  'socialShare.openDesignSection': 'Readable Studio 공유',
  'socialShare.openDesignTitle': 'GitHub의 Readable Studio',
  'socialShare.openDesignText': 'Readable Studio은 디자인 아티팩트를 생성, 편집, 배포 및 인수인계하기 위한 오픈소스 워크스페이스입니다.',
  'socialShare.openDesignCopyText': '{text}\n{url}',
  'socialShare.projectSection': '소셜 공유',
  'socialShare.projectTitle': '{title} · Readable Studio',
  'socialShare.projectText': 'Readable Studio으로 제작: {title}. 배포된 아티팩트를 확인하고 저장소에 스타를 남겨주세요: {repo}',
  'socialShare.projectCopyText': 'Readable Studio으로 제작: {title}\n{url}\nReadable Studio 저장소: {repo}',
  ...
  'preview.errorBody': '예제 HTML을 가져오지 못했습니다. Readable Studio이 실행 중인지 확인하고 다시 시도하세요.',
  'preview.retry': '다시 시도',
  ...
  'assistant.shareToOpenDesign': 'Readable Studio에 공유',
  ...
  'pet.codexBundled': '내장',
  'pet.codexBundledTitle': 'Readable Studio 에 내장 — 다운로드 불필요.',
  ...
  'settings.customInstructionsHint': '모든 채팅에서 Readable Studio이 따라야 할 고정 지침입니다. 사실, 선호 사항, 프로젝트 컨텍스트를 메모리로 저장하세요.',
  ...
  'settings.customInstructionsDesc': 'Readable Studio이 모든 채팅에서 따르는 고정 지침입니다. 저장된 메모리가 아니며, 사실, 선호 사항, 프로젝트 컨텍스트에는 메모리를 사용하세요.',
