...
export function AnalyticsProvider({ children }: { children: ReactNode }) {
  ...
  // Telemetry egress is hard-removed in this fork: `getAnalyticsClient` is a
  // permanent no-op (posthog-js never loads) and `bootstrapExceptionTracking`
  // no longer bridges the error tracker to any ingest endpoint, so the boot
  // effect that used to kick both off (and then resolve the daemon-stamped
  // installationId into `resolvedAnonId`) is gone. `resolvedAnonId` therefore
  // stays null and the header-injection effect below never installs — which
  // is exactly the desired end state. `setResolvedAnonId` is still wired into
  // setConsent/setIdentity in the value memo, so the state hook is retained.
