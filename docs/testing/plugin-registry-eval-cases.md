...
| REG-002 | 默认 official registry seed 是真实 catalog，不是空数组 | `plugins/registry/official/readable-studio-marketplace.json` 包含 bundled official entries，trust 为 `official`，且 `open-design/build-test` 可 resolve | `apps/daemon/tests/plugins-marketplaces.test.ts` |
